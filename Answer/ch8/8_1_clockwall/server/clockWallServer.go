...
//
//
// $ TZ=US/Eastern    ./client -port 8010 &
// $ TZ=Asia/Tokyo    ./client -port 8020 &
// $ TZ=Europe/London ./client -port 8030 &
// $ server NewYork=localhost:8010 Tokyo=localhost:8020 London=localhost:8030
...
func main() {
	...
	TZFlag := flag.String("TZ", "Asia/Shanghai", "时区")
	portFlag := flag.String("port", "8000", "the port to listen on")
	flag.Parse()

	// 加载指定时区
	location, err := time.LoadLocation(*TZFlag)
	if err != nil {
