...
import (
	...
	"errors"
	"fmt"
	"go_core_programming/Study/rpcStudy/stream_rpc_test/proto"
	"log"
	"net"
	"sync"
	"time"
...
// GetStream 实现服务端流模式
func (s *server) GetStream(req *proto.StreamRequest, serverStr proto.Greeter_GetStreamServer) error {
...
func (s *server) PutStream(clientStr grpc.ClientStreamingServer[proto.StreamRequest, proto.StreamResponse]) error {
	...
	clientStr.SendAndClose(&proto.StreamResponse{Data: "PutStream Response"})
	return nil
}
...
func (s *server) AllStream(allStream proto.Greeter_AllStreamServer) error {
	...
	go func() {
		...
		for i := 0; i <= 10; i++ {
			_ = allStream.Send(&proto.StreamResponse{Data: "你好我是服务器"})
