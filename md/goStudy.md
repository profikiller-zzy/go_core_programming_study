...
在`*_test.go`文件中，有三种类型的函数：测试函数、基准测试（benchmark）函数、示例函数。一个测试函数是以`Test`为函数名前缀的函数，用于测试程序的一些逻辑行为是否正确；go test命令会调用这些测试函数并报告测试结果是PASS或FAIL。

基准测试函数是以`Benchmark`为函数名前缀的函数，它们用于衡量一些函数的性能；go test命令会多次运行基准测试函数以计算一个平均的执行时间。

示例函数是以`Example`为函数名前缀的函数，提供一个由编译器保证正确性的示例文档。

go test命令会遍历所有的`*_test.go`文件中符合上述命名规则的函数，**生成一个临时的main包**用于调用相应的测试函数，接着构建并运行、报告测试结果，最后清理测试中生成的临时文件。
...
9.  测试单个文件，一定要带上被测试的原文件，例如：

```
go test -v cal_test.go cal.go
```

10. 测试单个方法可以使用 `-run`参数，参数`-run`对应一个正则表达式，只有测试函数名被它正确匹配的测试函数才会被`go test`测试命令运行：
...
# 十一、 `Goroutines`和`Channels`

## 1. `Goroutines` 

在Go语言中，每一个并发的执行单元叫作一个`goroutine`。当一个程序启动时，其主函数即在一个单独的`goroutine`中运行，我们叫它`main goroutine`。新的`goroutine`会用go语句来创建。在语法上，go语句是一个普通的函数或方法调用前加上关键字go。go语句会使其语句中的函数在一个新创建的`goroutine`中运行。例如：
...
func spinner(delay time.Duration) {
    for {
        for _, r := range `-\|/` {
            fmt.Printf("\r%c", r) // 打印 \r 时，光标会回到当前行的起始位置，
            					  // 后续输出会覆盖这一行的内容
            time.Sleep(delay)
        }
    }
...
`main goroutine`和`goroutine`的执行流程图如下所示：

![image-20241119191307088](image\image-20241119191307088.png)
...
这段代码实际上定义了一个返回时间的服务器，`Listen`函数创建了一个`net.Listener`的对象，这个对象会监听一个网络端口上到来的连接，在这个例子里我们用的是TCP的`localhost:8000`端口。当没有连接到来的时候，`listener`对象的`Accept`方法会直接阻塞，直到一个新的连接被创建，然后会返回一个`net.Conn`对象来表示这个连接。

这个服务器同时只能处理一个连接，假如有多个连接同时涌入，只有第一个连接服务器的客户端有响应内容，因为服务器组赛在`handleConn(conn)`上，下面是编写的用于接受服务器输入的客户端程序：
...
要同时处理多个客户端的连接，只需要将`handleConn(conn)`改为`go handleConn(conn)`

## 3. `Channels`

`goroutine`是Go语言程序的并发体的话，那么`channels`则是它们之间的通信机制。本质上来说`channels`的数据结构是队列。

并且`channels`线程安全，多`goroutine`访问时，不需要加锁，就是说 channel 本身就是线程安全的。

一个`channel`是一个通信机制，它可以让一个`goroutine`通过它给另一个`goroutine`发送值信息。每个`channel`都有一个特殊的类型，也就是`channels`可发送数据的类型。一个可以发送int类型数据的`channel`一般写为`chan int`。
...
1. channel是引用类型，和map类似。当复制一个channel或用于函数参数传递时，其实只是拷贝了一个channel引用，因此调用者和被调用者将引用同一个channel对象。和其它的引用类型一样，channel的零值也是nil。
2. channel 必须分配内存之后才能写入或读取数据, 即 make 后才能使用；

3. 两个相同类型的channel可以使用==运算符比较。如果两个channel引用的是相同的对象，那么比较的结果为真。一个channel也可以和nil进行比较。
4. channel支持close操作，用于关闭channel，随后于该channel的任何发送操作都会panic。对一个已经被close过的channel进行接收操作依然可以接受到之前已经成功发送的数据；如果channel中已经没有数据的话将产生一个零值的数据。

一个channel有发送和接受两个主要操作，都是通信行为。一个发送语句将一个值从一个`goroutine`通过channel发送到另一个执行接收操作的`goroutine`。发送和接收两个操作都使用`<-`运算符。在发送语句中，`<-`运算符分割channel和要发送的值。在接收语句中，`<-`运算符写在channel对象之前。一个不使用接收结果的接收操作也是合法的。

```go
ch <- x  // 发送语句
x = <-ch // 接收语句，从 channel 中接收一个值并赋值给x
<-ch     // 接受语句，值被丢弃了
```

### 11.3.1 不带缓存的`Channels`

一个基于无缓存Channels的发送操作将导致发送者`goroutine`阻塞，直到另一个`goroutine`在相同的Channels上执行接收操作，当发送的值通过Channels成功传输之后，两个`goroutine`可以继续执行后面的语句。反之，如果接收操作先发生，那么接收者`goroutine`也将阻塞，直到有另一个`goroutine`在相同的Channels上执行发送操作。

### 11.3.2 串联的`Channels`（Pipeline）

Channels也可以用于将多个`goroutine`连接在一起，一个Channel的输出作为下一个Channel的输入。这种串联的Channels就是所谓的管道（pipeline）

![img](image\ch8-01.png)
...
这样上面的程序会无休止地生成无穷数列，形似0、1、4、9、……，可以发送有限个数给channel之后关闭它：
...
我们可以在接收channel的值的语句中多接收一个值`ok`，`ture`表示成功从channels接收到值，`false`表示channel已经被关闭并且里面没有值可接收：

```go
x, ok := <-naturals
```

channel还可以支持`range`语句，range循环可直接在channels上面迭代，上面修改过后的pipeline如下：
...
并不需要关闭每一个channel。只有当需要告诉接收者`goroutine`，所有的数据已经全部发送时才需要关闭channel。不管一个channel是否被关闭，当它没有被引用时将会被Go语言的垃圾自动回收器回收。（不要将关闭一个打开文件的操作和关闭一个channel操作混淆。对于每个打开的文件，都需要在不使用的时候调用对应的Close方法来关闭文件。）

### 11.3.3 单方向的`Channels`

单方向的`Channels`，要么只用于接收，要么只用于发送。改进上面的程序：
...
调用counter（naturals）时，naturals的类型将隐式地从`chan int`转换成`chan<- int`。调用printer(squares)也会导致相似的隐式转换，这一次是转换为`<-chan int`类型只接收型的channel。这里并没有反向转换的语法：也就是不能将一个类似`chan<- int`类型的单向型的channel转换为`chan int`类型的双向型的channel。

### 11.3.4 带缓存的`Channels`
...
![img](image\ch8-02.png)

下面的例子展示了一个使用了带缓存channel的应用。它并发地向三个镜像站点发出请求，三个镜像站点分散在不同的地理位置。它们分别将收到的响应发送到带缓存channel，最后接收者只接收第一个收到的响应，也就是最快的那个响应。因此`mirroredQuery`函数可能在另外两个响应慢的镜像站点响应之前就返回了结果。（顺便说一下，多个`goroutines`并发地向同一个channel发送数据，或从同一个channel接收数据都是常见的用法。）
...
下面是`select`语句的一般形式，`select` 是一个控制结构，用于监听多个 `channel` 的操作（如读或写）：
...
`select` 会随机选择一个可以执行的 `case`；如果所有 `case` 都无法执行，`select` 会阻塞，直到其中一个 `case` 可以执行。如果定义了 `default`，当没有任何 `case` 可以执行时会立即执行 `default`，从而避免阻塞。

假设我们在模拟火箭点火，系统默认在10秒后给火箭点火，但是我们可以在十秒之前的任意时刻点击`Enter`让火箭提前发射，可以使用`select`来实现：
...
让我们来深究一下这个select语法块中发生了什么。首先是：

- `time.After` :

`time.After`会创建一个定时器和 `channel`，直到延迟结束或接收到数据，资源才会释放。`time.After(10 * time.Second)`在十秒钟之前，这个`channel`不会有任何数据可读，所以这个`case`会阻塞，在10秒后，这个 `channel`收到数据，这个case就可以执行了。

- `abort`：

在主函数中开了一个线程，假如用户按下一个按键(模拟用户按下按键)，按下之后会给abort发消息，在此之前abort是阻塞的，假如`select`没有`default`语句，如果所有case都阻塞则`select`会阻塞。
...
因为 `ch` 中没有数据，且没有其他协程向 `ch` 写入值，`default` 被执行，避免阻塞。

2. 超时机制

通过 `time.After` 实现超时功能：
...
如果 `ch` 没有在 2 秒内收到数据，`time.After` 的 `channel` 会触发，执行超时分支。
...
##### `go routine`中使用`recover`捕获错误

在 Golang 中，`recover` 用于从 `panic` 中恢复程序的正常执行。当你在 `goroutine` 中执行代码时，如果发生了 `panic`，可以使用 `recover` 捕获错误，从而防止整个程序崩溃。
...
- **`recover`** 只能在发生 `panic` 的同一个 `goroutine` 中被捕获。假如有一个`goroutine`发生了`panic`，在`main goroutine`中的`recover`语句无法捕获这个`panic`。
- 如果 `panic` 发生在一个 `goroutine` 中，而 `recover` 写在另一个 `goroutine` 中，则无法捕获该 `panic`。
- 为了捕获 `panic`，通常需要将 `recover` 放在 `defer` 中。
...
- 是一个接口，定义在`reflect`包中，用于表示 Go 类型的信息。
- 通过 `reflect.TypeOf` 获取变量的动态类型。
- `Type` 提供了许多方法来检查类型的组成（如结构体的字段、函数的参数等）。
- 返回值总是具体的类型（即实际存储的类型，而不是接口类型）。

![image-20241124160532653](image\image-20241124160532653.png)
...
`reflect.Value` 和 `interface{} `可以相互转换，并且他们都有各自的用处：
...
- `reflect.Value` 和 `interface{} `都能装载任意的值。不同的是，我们并不知道一个空接口类型的值有什么方法、其动态类型和动态值，并且只有当我们知道具体的动态类型时才能使用类型断言来访问内部的值。一个 Value 则有很多方法来检查其内容，无论它的具体类型是什么。



## 2. 反射的注意事项

1. `reflect.Value.Kind`，获取变量的类别，返回的是一个常量，这一部分查看文档：
...
// 因为golang是通过包来区分的，假设Student定义在包pkg1中(也可以定义在其他包中，则其Type就是
// package.Student)
```

3. 可以使用反射的机制来获取变量的值(并返回对应的类型)，要求数据类型匹配，比如说x是int，那么就应该使用`reflect.ValueOf(x).Int()`，而不能调用其他的方法，否则会报panic.
4. 正确理解Elem()
...
在 `Golang` 的 `reflect` 包中，`Elem` 方法用于获取一个值的底层具体内容。其行为根据 `v` 的 `Kind` 类型（如 `Interface` 或 `Ptr`）有所不同。

- 首先理解什么是“接口保管的值”：

在 Go 中，接口类型实际上是一个动态类型和值的组合。接口本身并不直接持有具体值，而是保管了这个值的一个引用（即具体值的封装）。

当你调用 `Elem`，它会解包接口，返回接口中实际存储的动态值。

- **“Value 封装”**：

调用 `reflect.ValueOf` 或 `reflect.Elem` 后返回的都是 `reflect.Value` 类型，它是 Go 反射中的核心类型，用于封装任意值。
...
func main() {
	var x interface{} = 42 // 接口保管了值 42
	v := reflect.ValueOf(x)

	// 获取接口保管的值
	elem := v.Elem() // 解包接口中的具体值
	fmt.Println(elem.Interface()) // 输出 42
	fmt.Println(elem.Kind())      // 输出 int
}
```

`x` 是一个接口类型，它保管了值 `42`；`reflect.ValueOf(x)` 返回封装接口的 `Value`。`Elem()` 解包接口，返回 `42` 的封装（`reflect.Value` 类型）。

- "返回v持有的指针指向的值的Value封装":
...
#### 使用反射来遍历结构体字段，调用结构体字段的方法，获取结构体标签的值
...
func TestStruct(a interface{}) {
	...
	val.Method(1).Call(nil) //获取到第二个方法并调用，这里的方法是Print
	// 								   排序的顺序是0.GetSum 1.Print 2.Set
	//调用结构体的第 1 个方法 0.GetSum
